...
Eigen::Vector3d  cal_delta_distence(Eigen::Vector3d odom_pose)
{
    ...
    //TODO:
    Eigen::Matrix3d last_T, now_T, delta_T;
    last_T << 
        cos(last_pos(2)), -sin(last_pos(2)), last_pos(0),
        sin(last_pos(2)), cos(last_pos(2)), last_pos(1),
        0, 0, 1;
    now_T << 
        cos(now_pos(2)), -sin(now_pos(2)), now_pos(0),
        sin(now_pos(2)), cos(now_pos(2)), now_pos(1),
        0, 0, 1;
    delta_T = last_T.inverse() * now_T;
    d_pos << delta_T(0, 2), delta_T(1, 2), atan2(delta_T(1, 0), delta_T(0, 0));
    //end of TODO:
