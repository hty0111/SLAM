...
void OccupancyMapping(std::vector<GeneralLaserScan> &scans, std::vector<Eigen::Vector3d> &robot_poses)
{
    ...
    for (int i = 0; i < scans.size(); i++)
    {
        ...
        for (int id = 0; id < scan.range_readings.size(); id++)
        {
            ...
            for (auto tempIndex : freeIndex)
            {
                int linearIndex = GridIndexToLinearIndex(tempIndex);
            ...
            // 更新hits
            int tempIndex = GridIndexToLinearIndex(mapIndex);
            int data = pMap[tempIndex];
...
int main(int argc, char **argv)
{
    ...
    std::string basePath = "/home/hty/Desktop/SLAM/Lidar_SLAM/HW7/OccupancyMappingProject/src/data";
