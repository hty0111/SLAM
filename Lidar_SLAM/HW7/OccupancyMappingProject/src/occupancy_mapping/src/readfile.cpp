...
void ReadPoseInformation(const std::string& path, std::vector<Eigen::Vector3d> &poses)
{
    ...
    while (std::getline(fin, line))
    {
        ...
        auto x = stringToNum<double>(results[0]);
        auto y = -stringToNum<double>(results[1]);
        auto theta = stringToNum<double>(results[2]);
                              ...
                              std::vector<GeneralLaserScan> &laserscans)
{
    ...
    {
        double anglei = -stringToNum<double>(result);
    ...
    while (std::getline(fin, line))
    {
        ...
        for (auto & result : results)
        {
            auto rangei = stringToNum<double>(result);
