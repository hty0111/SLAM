                                         ...
                                         double& height)
{
    ...
    {
        double weight = exp(-(x - nearPoints[i]).squaredNorm() / (m_h * m_h));
        projSum += weight * (x - nearPoints[i]).dot(nearNormals[i]);
        weightSum += weight;
    }
    height = projSum / (weightSum + 0.00001);
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    miu /= double(nearPoints.size());

    // 计算协方差
    Eigen::Matrix2d sigma;
    for (const auto & point : nearPoints)
    {
        Eigen::Vector2d err = point - miu;
        sigma += err * err.transpose();
    }
    sigma /= double(nearPoints.size());

    // 计算法向量，即最小特征值对应的特征向量
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix2d> eigen(sigma);
    Eigen::Matrix2d eigen_vectors = eigen.eigenvectors();
