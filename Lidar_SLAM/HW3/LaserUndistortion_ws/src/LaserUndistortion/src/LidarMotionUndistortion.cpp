...
public:
    ...
    void ScanCallBack(const champion_nav_msgs::ChampionNavLaserScanPtr& scan_msg)
    {
        ...
        //得到最终点的时间
        int beamNum = laserScanMsg.ranges.size();   // 激光点的数量
        ...
        //转换为pcl::pointcloud for visualization
    ...
    */
    ...
    {
       //TODO
       tf::Vector3 start_origin = frame_start_pose.getOrigin();
       tf::Vector3 end_origin = frame_end_pose.getOrigin();
       tf::Quaternion start_q = frame_start_pose.getRotation();
       tf::Quaternion end_q = frame_end_pose.getRotation();

       double inter_step = 1.0 / (beam_number - 1);
       for (int i = 1; i < beam_number; ++i)
       {
           tf::Vector3 i_origin = start_origin.lerp(end_origin, inter_step * i);   // 线性插值
           tf::Quaternion i_q = start_q.slerp(end_q, inter_step * i);  // 四元数插值
           tf::Pose frame_i_pose = tf::Transform(i_q, i_origin);

           // 基坐标下第i个点时的机器人位姿
           tf::Transform T_base_i = frame_base_pose.inverse() * frame_i_pose;

           // 第i个点在当前坐标系下的激光数据矢量P
           double i_range = ranges[startIndex + i];
           double i_angle = angles[startIndex + i];
           double i_x = i_range * cos(i_angle);
           double i_y = i_range * sin(i_angle);
           tf::Vector3 P_i(i_x, i_y, 1);

           // 转换到基坐标系
           tf::Vector3 P_base = T_base_i * P_i;

           // 重新发布
           ranges[startIndex + i] = sqrt(P_base.getX() * P_base.getX() + P_base.getY() * P_base.getY());
           angles[startIndex + i] = atan2(P_base.getY(), P_base.getX());
       }
    ...
    */
                           ...
                           tf::TransformListener * tf_)
    {
        ...
        {
            ROS_WARN("No Start Pose, Cannot Calib");
            return ;
        }

        if(!getLaserPose(frame_end_pose,ros::Time(end_time / 1000000.0), tf_))
        {
            ROS_WARN("No End Pose, Cannot Calib");
            return ;
        }
